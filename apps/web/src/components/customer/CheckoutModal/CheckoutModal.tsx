...
const CheckoutModal: React.FC<CheckoutModalProps> = ({ isOpen, onClose, form }) => {
  ...
  const renderInput = (label: string, name: string) => (
    <Input label={label} name={name} value={form.values[name]} onChange={form.onChange} error={getError(name)} />
  );
    ...
    <Backdrop visible={isOpen} onClick={onClose}>
      <div className={styles.modalContent} onClick={(e) => e.stopPropagation()}>
        <div className={styles.section}>
          ...
          <SelectCountry value={form.values.country} onChange={form.onChange} error={getError("country")} />
        </div>
